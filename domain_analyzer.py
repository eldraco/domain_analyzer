...
#  print "| www.mateslab.com.ar - Argentina                                      |"
#  print "+----------------------------------------------------------------------+"
  print("\nusage: {} -d <domain> <options>".format(sys.argv[0]))
  print("options:")
...
def user_input(prepend=''):
	if sys.version_info[0] < 3:
		return raw_input(prepend)
	else:
			...
			for ip in ip_list:
				...
				try:
					...
					for dicts in ip_registry:
						if 'IpCountry' in dicts:  # .has_key('sonmething') is removed in Python3, this works in both version
							has_country=True
					...
					has_ptr=False
					for dicts in ip_registry:
						if 'PTR' in dicts: # .has_key('sonmething') is removed in Python3, this works in both version
							has_ptr=True
...
def dns_request(domain):
	...
	try:
			...
			try:
				...
				for ip in domain_data['IpsInfo']:
					for dicts in domain_data['IpsInfo'][ip]:
						if 'Type' in dicts: # .has_key('sonmething') is removed in Python3, this works in both version
							if dicts['Type']=='NS':
			...
			try: 
				...
				for ip in domain_data['IpsInfo']:
					...
					if ip_net_block not in netblocks_checked:
						...
						if type(nmap_result) == bytes:
							nmap_result = nmap_result.decode('UTF-8')
		...
		#print('x =', x)      # ... Except when we get TypeError
		#print('y =', y)
	...
	try:
		...
		while amount:
			...
			try:
				...
				request_web = Request(results_web, headers={'User-Agent' : 'Mozilla/4.0 (compatible;MSIE 5.5; Windows NT 5.0)'}) #Python3 and Python2 compliant!
	...
	try:
		...
		try:
			while page_counter < 50 :
				...
				request = Request(results, headers={'User-Agent' : 'Mozilla/4.0 (compatible;MSIE 5.5; Windows NT 5.0)'}) #Python3 and Python2 compliant!)
				opener = build_opener()
...
def check_active_host():
	...
	try:
		...
		print (inst)           # __str__ allows args to printed directly
		#x, y = inst           # __getitem__ allows args to be unpacked directly
		#print ('x =', x)      # Except when getting TypeError
		#print ('y =', y)
		exc_type, exc_obj, tb = sys.exc_info()
		f = tb.tb_frame
		lineno = tb.tb_lineno
		...
		for ip in domain_data['IpsInfo']:
			...
			try:
				...
				for dicts in ip_registry:
					...
					elif 'HostName' in dicts: # .has_key('sonmething') is removed in Python3, this works in both version
						host_name=dicts.get('HostName')
					if 'HostUp' in dicts: # .has_key('sonmething') is removed in Python3, this works in both version
						...
						if 'True' in dicts['HostUp']:
							...
							nmap_command=shlex.split(nmap_command_temp)
							nmap_result_raw=Popen(nmap_command, stdout=PIPE).communicate()[0]
							if type(nmap_result_raw) == bytes:
								nmap_result_raw = nmap_result_raw.decode('UTF-8')
							nmap_result_raw=nmap_result_raw.split('\n')
							#
		...
		if option==1:
			...
			if len(domain_data['DomainInfo'])!=0:
				...
				for domdata in domain_data['DomainInfo']:
					...
					if 'Email' in domdata: # .has_key('sonmething') is removed in Python3, this works in both version
...
def find_robtex_domains():
	...
	try:
		...
		for ip in domain_data['IpsInfo']:
			...
			if robtex_domains or all_robtex:
				for dicts in ip_vect:
					if 'HostName' in dicts: # .has_key('sonmething') is removed in Python3, this works in both version
						hostname=dicts['HostName']
					if 'ZT' in dicts: # .has_key('sonmething') is removed in Python3, this works in both version
				...
				for dicts in ip_vect:
					if 'HostName' in dicts: # .has_key('sonmething') is removed in Python3, this works in both version
						hostname=dicts['HostName']
					if 'NS' in dicts: # .has_key('sonmething') is removed in Python3, this works in both version
						# Now we store the dns server in the list (for recursion) but avoid repiting! 
						# We use the IP because a lot of dns has several different names.
						if not ip in ns_servers_to_robtex: # .has_key('sonmething') is removed in Python3, this works in both version
							ns_servers_to_robtex[ip]=hostname        
							if debug:
								logging.debug('\tName server {0} ({1}) was added to be checked with robtex.'.format(ip, hostname))
		...
		for ip in ns_servers_to_robtex:
			...
			if hostname:
				...
				request_web = Request(results_web, headers={'User-Agent' : 'Mozilla/5.0 (X11; U; Linux x86_64; en-US) AppleWebKit/534.16 (KHTML, like Gecko) Chrome/10.0.648.205 Safari/534.16'})
				opener_web = build_opener()
				try:
					text = opener_web.open(request_web).read()
					if type(text) == bytes:
		...
		try:
			...
			for ip in domain_data['IpsInfo']:
				...
				for moredicts in ip_vect:
					if 'PortInfo' in moredicts:  # .has_key('sonmething') is removed in Python3, this works in both version
